...
#if defined(NDEBUG)
# error "Pricecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x24443f2156ab8a006debe26db2eecb76ee3467c865475591f32d3282bc52f486");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pricecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pricecoin Signed Message:\n";
...
{
    // Pricecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 18000); // Pricecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Pricecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Pricecoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pricecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd8beb01876c34d8e1b27393d5cd133e8d31aa432942e8da78560af604d70bec3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Z u c c bot confirmed 4/14/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cc3628d86672a688903cf542ad2883020058d1f80b52f4b3fa6291a06e57103fb925986e12f8f811e279cabadd2769d4655462c446dac5bd7f660ff163f2c7d4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523777906;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1524128091;

        if (fTestNet)
        {
            block.nTime    = 1523777888;
            block.nNonce   = 386424745;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9b1424e73b4b3da9b80510edfc59f8b76dd26eae0efab547b5356e4511aa0538"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbd, 0xd3 }; // Pricecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PricecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PricecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PricecoinMiner(CWallet *pwallet)
{
    printf("PricecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pricecoin-miner");
    ...
    try { loop {
        ...
        printf("Running PricecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PricecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PricecoinMiner, pwallet));
}
